...
NSString *const kGPUImageCRTFragmentShaderString = SHADER_STRING
(
...
#define TEX2D(c) pow(texture2D(inputImageTexture, (c)), vec4(inputGamma))
...
//#define distortion 0.1
 ...
 vec2 radialDistortion(vec2 coord) {
     vec2 cc = coord - 0.5;
     //distortion 0.1 default last value next line
 ...
 void main()
 {
     ...
     vec2 uv_ratio = fract(xy * inputSize) - vec2(0.5);
     
     // Snap to the center of the underlying texel.
     xy = (floor(xy * inputSize) + vec2(0.5)) / inputSize;
     ...
     float mod_factor = textureCoordinate.x * outputSize.x;
...
@implementation GPUImageCRTFilter

- (id)init;
{
    if (!(self = [super initWithFragmentShaderFromString:kGPUImageCRTFragmentShaderString]))
    {
		return nil;
    }
    
    CRTOutputSizeUniform = [filterProgram uniformIndex:@"outputSize"];
    CRTInputSizeUniform = [filterProgram uniformIndex:@"inputSize"];
    
    return self;
}

- (void)setCRTInputSize:(CGPoint)newInputSize
{
    _CRTInputSize = newInputSize;
    
    [self setPoint:newInputSize forUniform:CRTInputSizeUniform program:filterProgram];
}

-(void)setCRTOutputSize:(CGPoint)newOutputSize {
    _CRTOutputSize = newOutputSize;
    [self setPoint:newOutputSize forUniform:CRTOutputSizeUniform program:filterProgram];
    [self forceProcessingAtSize:CGSizeMake(newOutputSize.x, newOutputSize.y)];
}
